  ...
  if (!customerId || typeof customerId !== 'string') {
    ...
    if (isSubscription) {
      ...
      try {
        ...
        // Check if this is a payment code transaction
        ...
        if (lineItems.data.length > 0) {
          ...
          if (product?.metadata?.code) {
            ...
            const { error: updateError } = await supabase
              ...
              .eq('code', product.metadata.code);

            if (updateError) {
              console.error('Error marking payment code as used:', updateError);
            } else {
              console.info(`Marked payment code ${product.metadata.code} as used`);
            }
          }
        }
